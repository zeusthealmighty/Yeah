...
#if defined(NDEBUG)
# error "GoSuperSayayinCoin Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if(nPrevHeight ==      0) {nSubsidyBase = 1500000;}
    if(nPrevHeight ==      1) {nSubsidyBase = 0;}
    if(nPrevHeight ==    201) {nSubsidyBase = 1;}
    if(nPrevHeight ==   4501) {nSubsidyBase = 50;}
    if(nPrevHeight ==  25501) {nSubsidyBase = 45;}
    if(nPrevHeight ==  35501) {nSubsidyBase = 40;}
    if(nPrevHeight ==  60001) {nSubsidyBase = 35;}
    if(nPrevHeight == 100001) {nSubsidyBase = 30;}
    if(nPrevHeight == 125001) {nSubsidyBase = 25;}
    if(nPrevHeight == 150001) {nSubsidyBase = 20;}
    if(nPrevHeight == 200001) {nSubsidyBase = 15;}
    if(nPrevHeight == 250001) {nSubsidyBase = 10;}
    if(nPrevHeight %  500000 == 1) { nSubsidy /=2;}
...
{
    CAmount ret = blockValue*0.7; // start at 70%
    return ret;
}
...
void ThreadScriptCheck() {
    RenameThread("GoSuperSayayinCoin-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(GSS): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(GSS): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(GSS): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(GSS): spork is off, skipping transaction locking checks\n");
    }
