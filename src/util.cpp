...
//GoSuperSayayinCoin only features
...
const char * const BITCOIN_CONF_FILENAME = "GoSuperSayayinCoin.conf";
const char * const BITCOIN_PID_FILENAME = "GoSuperSayayinCoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "GoSuperSayayinCoin" is a composite category enabling all GoSuperSayayinCoin-related debug output
            if(ptrCategory->count(string("GoSuperSayayinCoin"))) {
...
#else
    const char* pszModule = "GoSuperSayayinCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GoSuperSayayinCoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GoSuperSayayinCoinCore
    // Mac: ~/Library/Application Support/GoSuperSayayinCoinCore
    // Unix: ~/.GoSuperSayayinCoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GoSuperSayayinCoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GoSuperSayayinCoinCore";
#else
    // Unix
    return pathRet / ".GoSuperSayayinCoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty GoSuperSayayinCoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override GoSuperSayayinCoin.conf
